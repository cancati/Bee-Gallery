...
const HomePage = () => {
    const {currentUser,photos,logout,widthSize,scrollHeight} = useAuth();
    ...
    return (
        <>
            {photos.length != 0 ?  <div className={'w-100 '} style={{backgroundColor:'#fed53d'}} >
                <NavBar user={user} />
                <div className={'flex-xl-wrap flex row w-100 p-4'} style={{backgroundColor:'#fed53d'}} >
                    {photos.map((item ) => {return (<Card item={item}/>)})}
                </div>
                    <div id={'Profil'} className={'position-sticky'} style={{width:'20%',minWidth:200,height:80 ,display:'flex',alignItems:'center',borderRadius:50,backgroundColor:'#f1a167',left:20,bottom: widthSize <= 800 && scrollHeight > 50  ? '90%' : 20}}>
                    ...
                    <div id={'Profil'} className={'position-sticky'} style={{width:80,minWidth:80,height:80 ,display:'flex',justifyContent:'center',alignItems:'center',borderRadius:30,backgroundColor:'#fed53d',left: '94%',bottom:widthSize <= 800 && scrollHeight > 50  ? '90%' : 20}}>
